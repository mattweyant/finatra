...
object ResponseBuilder {
  def apply(body: String): FinagleResponse =
    new ResponseBuilder(DefaultJacksonJsonSerializer).body(body).status(200).build

  def apply(status: Int, body: String): FinagleResponse =
    new ResponseBuilder(DefaultJacksonJsonSerializer).body(body).status(status).build

  def apply(status: Int, body: String, headers: Map[String, String]): FinagleResponse =
    new ResponseBuilder(DefaultJacksonJsonSerializer).body(body).status(status).headers(headers).build
}
